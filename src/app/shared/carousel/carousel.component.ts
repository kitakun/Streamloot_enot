...
import { NgxCarousel3dService } from './carousel.service';
...
export class CarouselComponent implements OnInit, OnDestroy, OnChanges {
  ...
  @Input() BeforeChange: Function;
  @Input() onLastSlide: Function;
  @Input() SlideChange: Function;
    ...
    return this.$carouselService
      ...
      .then((carousel) => {
        ...
        if (typeof outerHeight == 'number') {
          ...
          jquery('div.carousel-3d-container').css({ 'height': outerHeight });
        }

        this.$wrapper = jquery('div.carousel-3d');
        this.$wrapper.css({ 'max-width': outerWidth + 'px', 'height': outerHeight + 'px' });
  ...
  goSlide(index) {
    ...
    if (typeof this.BeforeChange === 'function') {
      keepChanging = this.BeforeChange({
  ...
  animationEnd() {
    ...
    if (typeof this.SlideChange === 'function') {
      this.SlideChange({
        index: this.carousel3d.currentIndex
      });
    }
